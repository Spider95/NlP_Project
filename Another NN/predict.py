...
def predict_unseen_data():
	...
	checkpoint_dir ="	D:\\My Data\\Studing\\Faculty\\4th level\\2's\\NLP\\Codes\\multi-class-text-classification-cnn-master\\trained_model_1523975993\\checkpoints\\model-400"    
	#checkpoint_file = tf.train.latest_checkpoint(checkpoint_dir )
	logging.critical('Loaded the trained model: {}'.format(checkpoint_dir))

	"""Step 1: load data for prediction"""
	#test_file = sys.argv[2]
	test_file="D:\\My Data\\Studing\\Faculty\\4th level\\2's\\NLP\\Codes\\multi-class-text-classification-cnn-master\\data\\test.json"
	...
	y_test = None
	if 'authar' in test_examples[0]:
		y_raw = [example['authar'] for example in test_examples]
		y_test = [label_dict[y] for y in y_raw]
		logging.info('The number of y_test: {}'.format(len(y_test)))
	check="D:\\My Data\\Studing\\Faculty\\4th level\\2's\\NLP\\Codes\\multi-class-text-classification-cnn-master\\trained_model_1523975993\\vocab.pickle"        
	vocab_path = os.path.join(check)
	...
	if y_test is not None:
		...
		with open('./data/test_prediction.json', 'w') as outfile:
